...
4. После этого датчик появляется в LAN / Home Assistant.

| | |
|---|---|
| SSID AP | `TH-Sens-Setup` |
| Пароль AP | `thsens5011` |
...
Сбрасываются сохранённые Wi‑Fi credentials и preference (в т.ч. калибровка). После сброса снова появится AP `TH-Sens-Setup`.
...
2. **Настройки → Устройства и службы → Добавить интеграцию → ESPHome**.
3. Хост: `th-sens-XXXXXX.local` или IP.
4. Вставьте **Encryption key** из `esphome/secrets.yaml` (`api_encryption_key`), с которым собирали прошивку.
5. После подключения `Status` станет **ON**.
...
- **Encryption key** обязателен (Noise). Без него HA 2023+ не подключится штатно.
- **`reboot_timeout: 0s`** — устройство **не** перезагружается, если клиент API (HA) долго не подключён. Это нужно для работы только с веб/`/prom`.
- Нативный API слушает порт **6053/tcp** — он должен быть доступен с хоста HA.
- OTA ESPHome: порт **8266/tcp**, пароль из `ota_password`.
...
1. Подключите телефон/ПК к Wi‑Fi **`TH-Sens-Setup`** (пароль `thsens5011`).
2. Откройте http://192.168.4.1/ (часто открывается сам).
...
# либо через Web OTA в интерфейсе устройства
```
...
- STA‑сеть и пароль **не** хранятся в исходниках и не попадают в публичный бинарник
- При необходимости смените пароль AP `TH-Sens-Setup` / `thsens5011` в `th-sens.yaml` и пересоберите
...
|---------|----------------|
| HA: Status OFF | API key, порт 6053, одна сеть с HA, mDNS |
...
| Нет в Wi‑Fi | Подключитесь к AP `TH-Sens-Setup` → http://192.168.4.1/ и задайте сеть |
| Не стабилен на БП / «нет данных» | 5V на **VIN**, ток ≥500 mA, общий GND; не подавать 5V на `3V3`; проверить, что устройство пингуется по IP/`*.local` |
| Калибровка не сохраняется | POST на `/number/Temp%20Offset/set` и `/number/Humidity%20Offset/set` (имя **с пробелами**); обновить UI с Ctrl+F5 |
